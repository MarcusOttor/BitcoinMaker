package com.freebitcoinmaker.satoshimining.bitcoinfaucet.screens
...
class MiningActivity : BaseActivity(), Runnable {
        ...
        when (v.id) {
            ...
            R.id.speed3 -> {
                if (threads !![currentPosition].speed > 3) {
                    ...
                    dialogsManager.showAdvAlertDialog(supportFragmentManager,
                            "Do you want to increase speed for this thread for ${(currentPosition + 1) * 3 * 1000} Satoshi?",
                            {
                                if (coinsManager.getCoins() >= ((currentPosition + 1) * 3 * 1000)) {
                                    ...
                                    dialogsManager.showAlertDialog(supportFragmentManager,
                                            "Not enough Satoshi!", { interstitial?.show() })
                                }
                            }, { interstitial?.show() })
                }
            }
            R.id.speed5 -> {
                if (threads !![currentPosition].speed < 5) {
                    dialogsManager.showAdvAlertDialog(supportFragmentManager,
                            "Do you want to increase speed for this thread for ${(currentPosition + 1) * 5 * 1000} Satoshi?",
                            {
    ...
    private fun updateSpeed() {
        miningSpeedText.text = (threads !![currentPosition].satoshi * threads !![currentPosition].speed).toString()
    }
    ...
    fun coreAdd() {
        if (preferencesManager.get(PreferencesManager.CORES, 1) > threads !!.size) {
            dialogsManager.showAdvAlertDialog(supportFragmentManager,
                    "Do you want to purchase a new thread for ${5000 * threads?.size !!} Satoshi?",
                    {
                        if (coinsManager.getCoins() >= (5000 * threads?.size !!)) {
                            ...
                            threadsText.text = "THREAD ${currentPosition + 1}"
                            updateSpeed()
                        } else {
    ...
    @OnClick(R.id.redeem, R.id.logout, R.id.free, R.id.menu,
            R.id.share, R.id.rateus, R.id.claiming, R.id.tickets, R.id.freeBtn, R.id.miningStarts)
    fun controls(v: View) {
        when (v.id) {
            R.id.redeem -> {
